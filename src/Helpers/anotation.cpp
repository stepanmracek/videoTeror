...
void Anotation::onMouseRegionSelect(int event, int x, int y, int flags, void *userdata)
{
    ...
    case Anotation::AnotationData::SecondClick:
        ...
        if (rect.contains(p))
        {
            data->result << data->currentFrame(rect).clone();
            qDebug() << "added";
        }
...
QVector<VideoTeror::BGRImage> Anotation::anotateRegions(cv::VideoCapture &video)
{
    AnotationData data;
...
void Anotation::extractFeatures(const QVector<VideoTeror::BGRImage> &textures, std::vector<cv::KeyPoint> &keypoints, cv::Mat &descriptors)
{
    cv::ORB featureExtractor;
    keypoints.clear();
    int n = textures.count();
    QVector<cv::Mat> descriptorsPerTexture(n);
    int r = 0;
    for (int i = 0; i < n; i++)
    {
