...
void Serialization::serialize(const std::string &filename, const std::vector<BGRImage> &images)
{
    cv::FileStorage storage(filename, cv::FileStorage::WRITE);
    storage << "images" << "[";

    for (const VideoTeror::BGRImage &img : images)
    {
        storage << img;
    }
...
void Serialization::deserialize(const std::string &filename, std::vector<BGRImage> &images)
{
    images.clear();
